...
def get_current_user(
    ...
    try:
        payload = decode_token(credentials.credentials)  # security.py의 decode_token 사용(발급과 동일 키/알고리즘)
    except Exception as e:
        logger.warning("JWT decode fail: %s", e)
        raise _unauth()

    sub = payload.get("sub")
    if not sub:
        logger.warning("JWT payload without sub: %s", payload)
        raise _unauth()

    user = _fetch_user_by_sub(db, sub)
    if not user:
        logger.warning("User not found by sub=%r", sub)
        raise _unauth()

    # user.user_id가 비어있지 않도록 보강
    if not getattr(user, "user_id", None):
        # 모델에 id/uuid가 있다면 채워줌(안전장치)
